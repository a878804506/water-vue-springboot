...
public interface WaterHistoryRepository extends BaseRepository<WaterInfo, Long> {
    ...
    @Query(value = "SELECT t.cid id, t.cname name, t.cost, t.remark, t.modify_time modifyTime, t1.name modifyName FROM t_water_waterinfo t LEFT JOIN t_sys_user t1 ON t.modify_by = t1.id WHERE t.modify_time BETWEEN DATE_FORMAT( ?1,'%Y-%m-%d %H:%i:%s')  and  DATE_FORMAT( ?2 ,'%Y-%m-%d %H:%i:%s') ORDER BY t.modify_time DESC ",nativeQuery = true)
