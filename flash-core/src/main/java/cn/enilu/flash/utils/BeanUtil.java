...
public class BeanUtil {
    ...
    public static <T> Map<String, Object> beanToMapDiy(T bean, String userNike) {
        Map<String, Object> map = Maps.newHashMap();
        if (bean != null) {
            BeanMap beanMap = BeanMap.create(bean);
            for (Object key : beanMap.keySet()) {
                map.put(key + "", beanMap.get(key));
                if ("type".equals(key.toString())) {
                    if ("1".equals(beanMap.get(key).toString())) {
                        map.put("type_", "普通开票");
                    } else {
                        map.put("type_", "包月");
                    }
                }
                if ("modifyTime".equals(key.toString())) {
                    String time = DateUtil.format((Date) beanMap.get(key), DateUtil.YMDHMS);
                    map.put("modifyTime_", time);
                }
            }
            map.put("modifyBy_", userNike);
        }
        return map;
    }
